...
if __name__ == '__main__':
    ...
    del db[db.get(ecut=ecut, formula='Li2O4', nkpts=6).id]

    Li2O = Compound('Li2O', 'mp-1960', db, xc, nkpts=nkpts, ecut=ecut, converged=True, extension='cif')
    Li2O2 = Compound('Li2O2', 'mp-841', db, xc, nkpts=nkpts, ecut=ecut, converged=True)  # validated!
    LiO2 = Compound('LiO2', 'mp-1018789', db, xc, nkpts=nkpts, ecut=ecut, U_correction={'O': ':p,0.33,0'}, converged=True)
    Li = Compound('Li', 'mp-1', db, xc, nbands=-10, nkpts=nkpts, ecut=ecut, converged=True)
    O2 = Compound('O2', 'mp-12957', db, xc, nkpts=nkpts, ecut=ecut, converged=True)

    # Li2O = Compound('Li2O', 'mp-1960', db, xc, ecut=800, converged=True, extension='cif')
    # Li2O2 = Compound('Li2O2', 'mp-841', db, xc, nkpts=6, ecut=550, converged=True)  # validated!
    # LiO2 = Compound('LiO2', 'mp-1018789', db, xc, nkpts=4, ecut=540, U_correction={'O': ':p,0.33,0'}, converged=True)
    # Li = Compound('Li', 'mp-1', db, xc, nbands=-10, ecut=520, converged=True)
    # O2 = Compound('O2', 'mp-12957', db, xc, nkpts=4, ecut=520, converged=True)
    ...
    print('epot_Li2O2', epot_Li2O2)
    print('epot_Li2O', epot_Li2O)
    print('epot_LiO2', epot_LiO2)
    ...
    print('ef li2o per atom:', ef_li2O / 3)  # goal: -2.067 eV

    ef_li2O2 = get_formation_energy(epot_Li2O2, 2, 2)
    print('ef li2o2 per atom:', ef_li2O2 / 4)  # goal: -1.651 eV

    ef_liO2 = get_formation_energy(epot_LiO2, 1, 2)
    print('ef lio2 per atom:', ef_liO2 / 3)  # goal: -1.033 eV
